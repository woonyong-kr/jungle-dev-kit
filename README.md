...
- `origin` remote의 HTTPS 토큰 또는 `GH_TOKEN`/`GITHUB_TOKEN`으로 GitHub API를 호출해 PR을 직접 생성합니다. reviewer 지정도 가능합니다.
- `gh auth login` 없이도 동작하도록 설계되어 있으며, 브랜치 push만 가능하면 PR 생성까지 이어집니다.

### 요구사항

- GitHub에 push 가능한 `origin` remote 또는 `GH_TOKEN`/`GITHUB_TOKEN`
