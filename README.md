...
| `@review` | 골드 `#FFD54F` | 코드 리뷰 포인트. 팀원의 코드를 리뷰할 때 자동 또는 수동으로 생성됩니다. 커밋 해시별로 그룹핑됩니다. | O |
...
**수정 버튼 (연필 아이콘)**: 사이드바에 표시되는 제목을 수정합니다. 파일 내 주석 원문은 변경되지 않습니다. 수정한 제목은 `.jungle-kit/annotations.json`에 저장됩니다.
...
같은 태그 유형 내에서 사이드바 항목을 드래그하여 순서를 변경할 수 있습니다. 단일 항목 드래그 시 두 항목의 위치가 교환(swap)되고, 여러 항목을 선택하여 드래그하면 선택한 항목들이 드롭 위치에 삽입됩니다. 변경된 순서는 `.jungle-kit/annotations.json`에 `sortOrder` 필드로 저장됩니다.
...
1. `.jungle-kit/scripts/clean-local.sh` -- awk 상태 머신 기반 필터 스크립트가 생성됩니다.
2. `git config filter.jungle-local.clean` -- 커밋 시 태그 주석을 제거하는 clean filter가 등록됩니다.
3. `git config filter.jungle-local.smudge` -- checkout 시 원본을 그대로 유지합니다 (`cat`).
4. `.gitattributes`에 `*.c filter=jungle-local`과 `*.h filter=jungle-local` 줄이 추가됩니다.

### 필터 제거 규칙

awk 상태 머신이 다음 세 가지 패턴을 모두 처리합니다:
...
- `.gitattributes`와 `.jungle-kit/` 디렉토리는 `.gitignore`에 자동 등록되어 원격 저장소에 올라가지 않습니다.
...
- 빨간색 gutter (왼쪽 3px): 같은 줄을 로컬에서도 수정함 (충돌 가능 영역)
...
- GitHub CLI (`gh`) 연동으로 PR을 직접 생성합니다. reviewer 지정도 가능합니다.
...
- 익스텐션 활성화 시 `.jungle-kit/styles/.clang-format` 파일이 자동 생성됩니다 (PintOS GNU 스타일 기반).
...
`.jungle-kit/config.json`의 `env.checks`에서 개별 항목을 `false`로 설정하여 비활성화할 수 있습니다. `env.showOnStartup`을 `false`로 설정하면 시작 시 자동 검증이 비활성화됩니다.
...
- `.jungle-kit/` 디렉토리
- `.jungle-kit/config.json` -- 프로젝트 설정 (커밋 컨벤션, 환경 검증 옵션, 스타일 설정)
- `.jungle-kit/reviews/`, `.jungle-kit/knowledge/`, `.jungle-kit/notes/` 서브 디렉토리
- `.jungle-kit/.gitignore`에 `annotations.json`, `keybindings.json`, `notes/` 자동 추가
...
| 설정 | 기본값 | 설명 |
|------|--------|------|
| `jungleKit.project` | `pintos` | 대상 OS 프로젝트 타입. `pintos`, `xv6`, `custom` 중 선택합니다. |
...
- 워크스페이스에 `.jungle-kit/config.json` 파일이 존재하는 경우
...
.jungle-kit/
  ...
  config.json            -- 프로젝트 설정
  .gitignore             -- annotations.json, keybindings.json, notes/ 제외
  scripts/
    clean-local.sh       -- git clean filter (awk 상태 머신 기반)
...
- PR 생성 시 GitHub CLI (https://cli.github.com)가 필요합니다
