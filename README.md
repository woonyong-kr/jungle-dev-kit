...
14. [Goal -- 장기 작업 목표 고정](#goal----장기-작업-목표-고정)
15. [Shadow Diff -- 팀원 변경사항 시각화](#shadow-diff----팀원-변경사항-시각화)
16. [PR 만들기](#pr-만들기)
17. [코딩 스타일 검사](#코딩-스타일-검사)
18. [환경 검증](#환경-검증)
19. [단축키 설정](#단축키-설정)
20. [API 키 관리](#api-키-관리)
21. [프로젝트 초기화](#프로젝트-초기화)
22. [설정](#설정)
23. [활성화 조건](#활성화-조건)
24. [파일 구조](#파일-구조)
25. [요구사항](#요구사항)
...
6. `Cmd+Shift+P`에서 `Goal 설정`을 실행해 이번 작업의 목표와 완료 기준을 고정합니다.
7. 상태바의 `Goal` 표시를 클릭하면 수정, 복사, 완료, 삭제를 빠르게 할 수 있습니다.
8. `git diff`를 실행해보면 태그 주석이 diff에 나타나지 않는 것을 확인할 수 있습니다.
...
Codex CLI의 `/goal`처럼 현재 작업 목표를 워크스페이스에 고정해두는 기능입니다. Goal은 `.annotation/goal.json`에 로컬로 저장되며 git에는 포함되지 않습니다.

### 제공 명령

- `Goal 설정` -- 현재 작업의 목표와 완료 기준을 저장합니다.
- `Goal 보기` -- 현재 Goal을 확인하고 수정, 복사, 완료, 삭제를 수행합니다.
- `Goal 완료 처리` -- 활성 Goal을 완료 상태로 옮기고 상태바에서 제거합니다.
- `Goal 삭제` -- 현재 Goal을 바로 비웁니다.

### 동작 방식

- 활성 Goal이 있으면 상태바 왼쪽에 `$(target)` 아이콘과 함께 항상 표시됩니다.
- 상태바를 클릭하면 현재 Goal 관리 메뉴가 열립니다.
- AI 커밋 메시지 생성, AI PR 생성, AI 리뷰 설명 생성 시 활성 Goal이 함께 전달되어 현재 작업 맥락을 유지합니다.
...
- `.annotation/.gitignore`에 `annotations.json`, `keybindings.json`, `goal.json`, `notes/` 자동 추가
...
| `jungleKit.goal.showStatusBar` | `true` | 활성 Goal을 상태바에 표시할지 여부입니다. |
| `jungleKit.goal.includeInAI` | `true` | 활성 Goal을 AI 커밋/PR/리뷰 프롬프트에 포함할지 여부입니다. |
