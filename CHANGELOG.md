...
### Fixed
- **같은 줄 다중 태그 충돌 수정**: 수동 태그 삽입 시 같은 줄의 virtual(@review) 어노테이션 줄 번호가 조정되지 않아 위치가 꼬이던 버그 수정
- **scanDocument auto-review 충돌 해소**: 같은 줄에 수동 태그와 virtual 태그가 공존할 때 virtual 태그를 한 줄 아래로 이동하여 충돌 방지
- **deleteAnnotationAtCursor 다중 태그 우선순위**: 같은 줄에 여러 타입의 태그가 있을 때 수동(파일 내) 태그를 우선 삭제하도록 변경
- **P4 파일 복원 + 메모리 동기화**: git 필터에 의한 어노테이션 소실 시 파일에 주석을 복원하고, 복원 완료 후 재스캔하여 메모리와 파일 줄 번호를 동기화
- **P4 부분 소실 재진입 방지**: `_p4Restoring` 가드를 모든 문서 이벤트 핸들러에 적용하여 복원 중 재스캔으로 인한 경쟁 조건 제거
- **부분 소실 복구 소비 기반 매칭**: 동일 type:content의 어노테이션이 여러 개일 때 줄 번호 우선 매칭으로 정확히 누락된 항목만 복원 (기존: Set 비교로 잘못된 항목이 복원될 수 있었음)
- **clearAllAnnotations 필터 삭제 후 줄 번호 재동기화**: 특정 타입만 삭제 시 같은 파일의 남은 어노테이션 줄 번호가 밀리는 문제를 재스캔으로 해결
- **scanWorkspaceAnnotations P4 트리거**: annotations.json에만 있고 파일에서 사라진 어노테이션도 스캔하여 P4 복원을 트리거

## [0.29.1] - 2026-05-06

### Fixed
- **P4 보존 조건 isDirty 제거**: git 필터가 파일을 교체한 뒤 사용자가 편집을 시작하면 `isDirty=true`로 보존이 해제되어 어노테이션이 소실되던 버그 수정 — `fromSave`만으로 보존 해제를 판단하도록 변경

## [0.29.0] - 2026-05-06

### Fixed
- **smudge 필터 offset 버그 수정**: 어노테이션 2개 이상인 파일에서 clean→smudge 사이클마다 위치가 점진적으로 밀리던 근본 원인 제거 — offset 누적 로직 삭제, 원본 줄 번호 직접 사용
- **smudge 필터 P1 인접 검사 수정**: 연속된 어노테이션이 복원되지 않던 버그 수정 — 인접 줄(±1) 검사를 정확한 삽입 위치만 검사하도록 변경
- **auto-review 자기 커밋 스킵 제거**: 1인 개발 환경에서 모든 auto-review가 차단되던 문제 해결 — `commitAuthor === myName` 가드 제거
...
- **P4**: `scanDocument` 캐스케이드 와이프 방지 — `fromSave`/`isDirty` 조건으로 git 필터에 의한 일시적 제거와 사용자 의도적 삭제를 정확히 구분
- **P5**: `_deletionGuard`를 타임아웃 기반에서 소비(consume) 기반으로 전환 — 다음 scanDocument에서 자동 제거되어 타이밍 의존성 완전 제거
- **P6**: smudge 필터 멀티라인 어노테이션 복원 시 빈 줄 생성 → 항상 한 줄짜리 주석으로 복원

### Removed
- **죽은 코드 삭제**: `annotationReconciler.ts` — 어디서도 import하지 않는 미사용 파일 제거
